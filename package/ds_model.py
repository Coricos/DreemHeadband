...
class DS_Model:
    ...
    def __init__(self, input_db, channel, marker=None):
        ...
        self.cls = channel
        # Output definition
        if marker: self.spc = './models/SPC_{}_{}.ks'.format(self.cls, marker)
        else: self.spc = './models/SPC_{}.ks'.format(self.cls)
        if marker: self.out = './models/DSN_{}_{}.ks'.format(self.cls, marker)
        else: self.out = './models/DSN_{}.ks'.format(self.cls)
    ...
    def train_spatial(self, epochs=100, batch=64, ini_dropout=0.5, decrease=50):
        ...
        inp = Reshape((inp._keras_shape[1], 1))(inp)
        ...
        chk = ModelCheckpoint(self.spc, monitor='loss', save_best_only=True, save_weights_only=False)
        ...
        print('# Small feature map shape:', s_mod._keras_shape)

        # Build the channel aimed at longer patterns
        l_mod = Reshape((inp._keras_shape[1], 1))(inp)
        l_mod = Conv1D(64, f_s*4, strides=13, **arg)(l_mod)
        l_mod = PReLU()(l_mod)
        ...
        print('# Large feature map shape:', l_mod._keras_shape)
        ...
        # Memory efficiency
        del ros, vec, lab, inp, arg, drp, ear, chk, s_mod, l_mod, space, model, optim
    ...
    def train_temporal(self, epochs=100, batch=64, ini_dropout=0.5, decrease=50):
        ...
        chk = ModelCheckpoint(self.spc, monitor='val_loss', save_best_only=True, save_weights_only=False)

        model = space(inp)
        print('# Convolutional feature space:', model._keras_shape)
        ...
        tempo = PReLU()(tempo)
        tempo = AdaptiveDropout(drp.prb, drp)(tempo)
        print('# Temporal feature map shape:', tempo._keras_shape)
        ...
        print('# Intermediate map:', model._keras_shape)
