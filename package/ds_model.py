...
class AutoEncoder:
    ...
    def build(self, dropout, decrease):
        ...
        mod = Conv1D(16, 16, padding='same', **arg)(mod)
        mod = BatchNormalization()(mod)
        mod = PReLU()(mod)
        mod = MaxPooling1D(pool_size=5)(mod)
        mod = AdaptiveDropout(self.drp.prb, self.drp)(mod)
        mod = Conv1D(16, 6, padding='same', **arg)(mod)
        mod = BatchNormalization()(mod)
        mod = PReLU()(mod)
        mod = MaxPooling1D(pool_size=5)(mod)
        enc = GaussianNoise(1e-2)(mod)
        print('# ENCODER Latent Space', enc._keras_shape)

        mod = Conv1D(16, 6, padding='same', **arg)(enc)
        mod = BatchNormalization()(mod)
        mod = PReLU()(mod)
        ...
        mod = PReLU()(mod)
        mod = UpSampling1D(5)(mod)
        mod = Conv1D(1, 16, padding='same', **arg)(mod)
        mod = AdaptiveDropout(self.drp.prb, self.drp)(mod)
...
class VAutoEncoder:
    ...
    def __init__(self, channel, storage='./dataset'):
        ...
        self.dtb = storage
        # Path for checkpoint 
        self.ate = './models/VAE_{}.weights'.format(channel)
        self.his = './models/HIS_VAE_{}.history'.format(channel)
        self.enc = './models/VEN_{}.weights'.format(channel)
    ...
    # dropout refers to the amount of dropout to set in the model
    # decrease refers to the number of epochs for decreasing ratio
    def build(self, latent_dim, dropout, decrease):
        ...
        self.inp = Input(shape=(self.raw_t.shape[1], ))
        e_0 = Dense(self.inp._keras_shape[1] // 3, **arg)(self.inp)
        mod = BatchNormalization()(e_0)
        mod = PReLU()(mod)
        mod = AdaptiveDropout(self.drp.prb, self.drp)(mod)
        e_1 = Dense(mod._keras_shape[1] // 3, **arg)(mod)
        ...
        mod = PReLU()(mod)
        mod = AdaptiveDropout(self.drp.prb, self.drp)(mod)
        mod = Dense(e_0._keras_shape[1], **arg)(mod)
        mod = BatchNormalization()(mod)
        mod = PReLU()(mod)
        mod = AdaptiveDropout(self.drp.prb, self.drp)(mod)
